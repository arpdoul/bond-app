...
function StreamTicker({ isActive }) {
  ...
  useEffect(() => {
    if (!isActive) return;
    const interval = setInterval(() => setDisplay(prev => prev + (Math.random() * 0.000008)), 80);
...
function AgentCard({ agent, status, task, totalSpent }) {
  const statusColor = status==="active"?agent.color:status==="idle"?"#444":"#FF6B35";
  return (
    <div style={{ background:"rgba(255,255,255,0.03)", border:`1px solid ${status==="active"?agent.color+"33":"#1a1a1a"}`, borderRadius:12, padding:"16px 18px", transition:"all 0.4s ease", position:"relative", overflow:"hidden" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <span style={{ fontSize:20, color:agent.color }}>{agent.icon}</span>
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:"#fff", letterSpacing:"0.05em" }}>{agent.name}</div>
            <div style={{ fontSize:10, color:"#555", textTransform:"uppercase", letterSpacing:"0.1em" }}>{agent.role}</div>
          </div>
        </div>
        <div style={{ fontSize:9, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.15em", color:statusColor, padding:"3px 8px", border:`1px solid ${statusColor}44`, borderRadius:4 }}>{status}</div>
      </div>
  ...
  return (
    <div style={{ opacity:show?1:0, transform:show?"translateX(0)":"translateX(20px)", transition:"all 0.4s ease", background:"rgba(255,255,255,0.02)", border:"1px solid #1f1f1f", borderRadius:10, padding:"14px 16px", marginBottom:8 }}>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
        ...
        <span style={{ fontSize:9, color:s.status==="settled"?"#00FFB2":"#FF6B35", textTransform:"uppercase", letterSpacing:"0.1em", border:`1px solid ${s.status==="settled"?"#00FFB244":"#FF6B3544"}`, padding:"2px 6px", borderRadius:3 }}>{s.status}</span>
      </div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
...
export default function BondApp() {
  ...
  const { connect, wallet, balance, loading, error } = useCircleWallet();
  const [walletConnected, setWalletConnected] = useState(false);
  const [walletAddr, setWalletAddr] = useState("");
  const intervalRef = useRef(null);
  const logsEndRef = useRef(null);
  const connectWallet = async () => { const token = await connect(); if (token) { setWalletConnected(true); setWalletAddr(wallet?.address || generateAddress()); } };
  ...
  const S = {
    app:{ minHeight:"100vh", background:"#080808", color:"#fff", fontFamily:"sans-serif", display:"flex", flexDirection:"column" },
    header:{ padding:"16px 24px", borderBottom:"1px solid #111", display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(8,8,8,0.95)", position:"sticky", top:0, zIndex:100 },
    logo:{ width:32, height:32, borderRadius:8, background:"linear-gradient(135deg,#00FFB2,#00a876)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, fontWeight:900, color:"#080808" },
    tabs:{ display:"flex", gap:4, padding:"12px 24px 0", borderBottom:"1px solid #111" },
    content:{ flex:1, padding:"24px", maxWidth:900, margin:"0 auto", width:"100%" },
    card:{ background:"rgba(255,255,255,0.02)", border:"1px solid #141414", borderRadius:12, padding:"16px 18px" },
    statLabel:{ fontSize:10, color:"#444", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:8 },
      ...
      <header style={S.header}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          {walletConnected
            ...
            : <button onClick={connectWallet} style={{ background:"transparent", border:"1px solid #222", color:"#888", fontSize:11, fontWeight:600, padding:"6px 14px", borderRadius:6, cursor:"pointer", textTransform:"uppercase" }}>Connect Wallet</button>
          }
        </div>
      </header>
      <div style={S.tabs}>
          ...
          <div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:24 }}>
              {[{label:"Total USDC Spent",value:`$${totalSpent.toFixed(4)}`,color:"#00FFB2"},{label:"Payments Made",value:`${logs.length}`,color:"#A78BFA"},{label:"Settlements",value:`${settlements.filter(s=>s.status==="settled").length}`,color:"#FF6B35"}].map((s,i) => (
                <div key={i} style={S.card}><div style={S.statLabel}>{s.label}</div><div style={{...S.statVal, color:s.color, fontFamily:"monospace"}}>{s.value}</div></div>
              ))}
            </div>
            ...
            <div style={{...S.card, marginBottom:16}}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:12, marginBottom:12 }}>
                <div>
                  <label style={{ fontSize:11, color:"#444", letterSpacing:"0.08em", textTransform:"uppercase", display:"block", marginBottom:6 }}>Task</label>
                  <select value={task} onChange={e=>setTask(e.target.value)} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid #1f1f1f", borderRadius:8, color:"#fff", fontFamily:"sans-serif", fontSize:13, padding:"10px 14px", width:"100%", outline:"none" }}>
                    {TASKS.map(t => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div style={{ width:130 }}>
                  <label style={{ fontSize:11, color:"#444", letterSpacing:"0.08em", textTransform:"uppercase", display:"block", marginBottom:6 }}>USDC Budget</label>
